...
private:
  ...
  std::any default_value_;        ///< Default value returned when no matching
                                  ///< timestamp is found
public:
  /**
   ...
   * @param default_value Default value returned when no matching timestamp is
   * found
   */
  DelayAny(PreciseSeconds max_time_delay, PreciseSeconds nominal_dt,
...
public:
  RateLimiter(double maximal_rate, PreciseSeconds clock,
              ...
              double eps = 1e-6)
      : maximal_rate_(maximal_rate), output_(initial_value), eps_(eps),
        max_dt_(max_dt), last_update_time_(clock) {}
...
public:
  FirstOrderLPF(PreciseSeconds clock, double freq_cutoff, double freq_sampling,
                double initial_value = 0.0, PreciseSeconds max_dt = 0.05,
                double eps = 1e-6)
      :                              /*
        ...
        dt_(1 / freq_sampling), yk_minus_1_(initial_value), yk_(initial_value),
        uk_minus_1_(0.0), alfa_((2 - wc_ * dt_) / (2 + wc_ * dt_)),
        beta_(wc_ * dt_ / (2 + wc_ * dt_)), max_dt_(max_dt),
        last_update_time_(clock), eps_(eps) {}
  void Reset(PreciseSeconds clock, double initial_value) {
    output_ = initial_value;
    last_update_time_ = clock;
  }
...
public:
  SecondOrderLPF() {}
  SecondOrderLPF(PreciseSeconds clock, double freq_cutoff, double dumping_ratio,
