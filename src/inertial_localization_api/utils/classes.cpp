...
#include "inertial_localization_api/utils/functions.h"

BufferAny::BufferAny(int max_size) : maximal_size_(max_size) {}
...
std::any BufferAny::GetFirst() { return values_.front(); }
...
std::tuple<int, double> BufferAny::ClosestValue(double target) {
  ...
  for (int i = 0; i < this->Size(); i++) {
    delta_vec.push_back(abs(std::any_cast<double>(this->GetValue(i)) - target));
  }
  int idx =
      std::min_element(delta_vec.begin(), delta_vec.end()) - delta_vec.begin();
  return std::make_tuple(idx, std::any_cast<double>(values_[idx]));
}
int BufferAny::Size() { return values_.size(); }
DelayAny::DelayAny(PreciseSeconds max_time_delay, PreciseSeconds nominal_dt,
                   std::any default_value)
    : stored_values_(BufferAny(max_time_delay / nominal_dt)),
      clock_(BufferAny(max_time_delay / nominal_dt)),
      max_time_delay_(max_time_delay), default_value_(default_value) {}
...
std::any DelayAny::GetDelayedValue(PreciseSeconds delayed_clock) {
  ...
  } else if (std::any_cast<PreciseSeconds>(clock_.GetFirst()) > delayed_clock) {
    return default_value_;
  } else {
...
void DelayAny::Test() {
  ...
  for (int i = 0; i < t.size(); ++i) {
    ...
    delayed_signal.push_back(
        std::any_cast<double>(delay_obj.GetDelayedValue(t[i] - nominal_delay)));
  }
  ...
  }
  {
    ...
    std::vector<std::pair<std::string, std::vector<double>>> dict_for_csv =
        PackVectorsToNameValuePairs(headers, data);
  ...
  }
  {
    ...
    std::vector<std::pair<std::string, std::vector<double>>> dict_for_csv =
        PackVectorsToNameValuePairs(headers, data);
    PrintToCSV(delay_test_results_dir / "delayed_signal", dict_for_csv, true);
  }
  std::string system_cmd = "python ../Tests/python/plot_inputs.py --path " +
                           (delay_test_results_dir / "signal").string() +
                           " --path2 " +
                           (delay_test_results_dir / "delayed_signal").string();
  std::cout << system_cmd << "\n";
  system(system_cmd.c_str());
}
SecondOrderLPF::SecondOrderLPF(PreciseSeconds clock, double freq_cutoff,
                               double dumping_ratio, double initial_value,
                               double initial_derivative, double max_dt,
                               double eps) {
...
void SecondOrderLPF::Reset(PreciseSeconds clock, double initial_value,
...
void SecondOrderLPF::StepTest() {
  std::vector<PreciseSeconds> t1 =
      Arange((PreciseSeconds)0, (PreciseSeconds)10, (PreciseSeconds)0.001);
  std::vector<PreciseSeconds> t2 =
      Arange((PreciseSeconds)50, (PreciseSeconds)60, (PreciseSeconds)0.001);
