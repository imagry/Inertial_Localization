...
double VectorNorm2D(const std::vector<double> &vec2d) {
  if (vec2d.size() != 2) {
    throw std::invalid_argument(
        "VectorNorm2D: Input vector must have exactly 2 elements");
  }
...
std::vector<double> SubtractVectors(const std::vector<double> &vec1,
...
std::vector<bool>
NumbersInRange(const std::vector<std::vector<double>> &nums,
...
Eigen::Matrix3d Mpsi(PreciseRadians psi) {
  Eigen::Matrix3d Mpsi;
  Mpsi << std::cos(psi), std::sin(psi), 0, -std::sin(psi), std::cos(psi), 0, 0,
      0, 1;
  return Mpsi;
}
...
Eigen::Matrix<double, Dynamic, 2>
ProjectPoints2D(Matrix3d T12, Eigen::Matrix<double, Dynamic, 2> points) {
  /*
  ...
  */
  if (points.rows() <= 0) {
    throw std::invalid_argument(
        "ProjectPoints2D: Input points matrix must have at least one row");
  }
...
double Vector_2_norm(const std::vector<double> &vec) {
  ...
  if (vec.size() != 2) {
    throw std::invalid_argument(
        "Vector_2_norm: invalid dimensions (must be 2)");
  }
  return sqrt(pow(vec[0], 2) + pow(vec[1], 2));
}

std::vector<double>
Norm_nX2_array(const std::vector<std::vector<double>> &nX2_array) {
  // like np.linalg.norm(nX2_array, axis=1)
  if (nX2_array.empty() || nX2_array[0].size() != 2) {
    throw std::invalid_argument(
        "Norm_nX2_array: invalid dimensions (must be nx2)");
  }
