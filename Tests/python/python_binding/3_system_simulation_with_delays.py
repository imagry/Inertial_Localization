...
from control_module import ControlAPI, AHRSLocHandler
...
def run_system_simulation(simulation_params, desired_traj, velocity_profile_KPH, odometer_signal, heading_measurement_errors, delay_fluctuations):
    def observation_model():
        ...
        idx_start = Functions.project_point_on_path(point=[vehicle_obj.x, vehicle_obj.y], path=desired_traj)
        idx_end = np.argmin(
        ...
        s_sparse = np.arange(desired_traj_s[idx_start], desired_traj_s[idx_end],
                             step=simulation_params["motion_planing_path_spacing"])
        cond4 = s_sparse.shape[0] <= 5  # check there's at least 5 points or end simulation
        if cond4:
            ...
            desired_traj_x_sparse = np.interp(x=s_sparse, xp=desired_traj_s, fp=desired_traj_x)
            desired_traj_y_sparse = np.interp(x=s_sparse, xp=desired_traj_s, fp=desired_traj_y)
            desired_traj_sparse_nav = np.vstack([desired_traj_x_sparse, desired_traj_y_sparse]).T
    ...
    if simulation_params['model'] == 'Dynamic':
        ...
        vehicle_obj = VehicleKinemaicModel(x=desired_traj_x[0] + error_x, y=desired_traj_y[0] + error_y, psi=desired_traj_psi[0],
                                           vehicle_params=vehicle_params, simulation_params=simulation_params,
                                           steering_uncertainty_factor=1.0, lr_uncertainty_factor=1.0, WB_uncertainty_factor=1.0)
    else:
        raise 'invalid model type'
    cpp_control_obj = ControlAPI("vehicle_config.json", "control_config.json")
    cpp_localization_obj = AHRSLocHandler("vehicle_config.json", "control_config.json")
    cpp_localization_obj.ResetVehicleState(t[0], [vehicle_obj.x, vehicle_obj.y, vehicle_obj.psi, vehicle_obj.vx])
    vehicle_obj.vx = 0.0
    ...
    # P_est_ego = cpp_localization_obj.GetPosition()
    ...
    P_est = [cpp_localization_obj.GetPosition()]
    front_slip_angle = []
    rear_slip_angle = []
    delta = []
    ef = []
    ...
    while not stop_condition:
            ...
            if not cond4:  # there's at least 5 points or end simulation
                ...
                if observation_is_valid:
                    ...
                    if i == 0:
                        ...
                        delta_i = cpp_control_obj.GetSteeringCmd() / vehicle_params["steering_ratio"]
        ...
        if observation_is_valid:
            ...
            steering_cmd = cpp_control_obj.GetSteeringCmd() / vehicle_params["steering_ratio"] # filtered steering  wheel command mirrored to wheels
            vehicle_obj.update(a=0, delta=steering_cmd) # assuming no tracking error at this point
            ...
            delta_i = 0.0
            odometer_i = 0.0
        cpp_control_obj.UpdateSteeringWheel(cpp_control_obj.GetSteeringCmd(), t[i])
        ...
        if np.mod(i, ndt_animation) == 0 and simulation_params['animate']:
            if simulation_params['animation_mode'] == 'GT_navigation_frame':
                ...
                plotter_obj.update_plot(current_car_position_x=vehicle_obj.x,
                                        ...
                                        steering=cpp_control_obj.GetSteeringCmd() / vehicle_params["steering_ratio"],
                                        motion_planning_traj_raw=desired_traj_sparse_nav_delayed,
                                        motion_planning_traj_processed=control_ref_traj_fixed_to_nav,
                                        lateral_error=ef[-1],
                                        t_i=ti,
                                        steering_cmd_raw=cpp_control_obj.GetDelta(),
                                        steering_cmd_filtered=cpp_control_obj.GetSteeringCmd() / vehicle_params["steering_ratio"],
...
if __name__ == '__main__':
    scenario = 'ellipse'  # 'sin', 'straight_line', 'square', shiba, random_curvature, eight, ellipse
    desired_traj = Functions.calc_desired_path(scenario, traj_noise=[0.05,0.25], plot_results=False)
    simulation_params = {'dt': 0.01, 't_end': 20, 'ego_frame_placement': 'front_axle', 'velocity_KPH': 15, 'time_delay': 0.0,
                         ...
                         "motion_planning_horizon": 20, "motion_planing_path_spacing": 0.1, "control_interp_spacing": 0.1,
                         'velocity_variations': 0.1, 'heading_drift_noise': 1.0*0, 'delay_fluctuations': 0.05*0,
