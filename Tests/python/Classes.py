...
class Trip:
    ...
    def __init__(self, path):
        ...
        self.imu.yaw = Functions.continuous_angle(self.imu.yaw, "deg")  # ensure yaw is continuous
...
class FirstOrderLPF_WithRateLimit:
    def __init__(self, clock, freq_cutoff, freq_sampling, max_rate,
        ...
        freq_sampling, freq_cutoff in Hz
        """
        self.wc = freq_cutoff * 2 * np.pi # cut off frequency in rad/sec
        self.dt = 1 / (freq_sampling)
        self.y_k_minus_1 = initial_value
        self.yk = initial_value
        self.u_k_minus_1 = None
        self.alpha = (2 - self.dt * self.wc) / (2 + self.dt * self.wc)
    ...
    def update(self, uk, clock):
        ...
        if clock - self.last_update_time > self.max_dt:
            self.initialize_clock(clock)
            return self.y_k_minus_1
        self.dt = clock - self.last_update_time
        self.last_update_time = clock
        input_to_integrator = self.wc * (uk - self.y_k_minus_1)
        input_to_integrator_limited = limit_x(input_to_integrator, self.max_rate) # this is the output derivative
    ...
    def step_test(self):
        ...
        plt.plot(t, u, t, y)
        plt.legend(["u", "y"]), plt.ylabel('$x$')
        plt.scatter(t, u, s=5, color='k')
        plt.scatter(t, y, s=5, color='k')
        plt.grid(True)
        plt.subplot(3, 1, 2, sharex=h)
        plt.plot(t, u_dot, t, y_dot)
...
class SecondOrderLPF_WithRateAndAccLimit:
    ...
    def step_test(self):
        ...
        filter = SecondOrderLPF_WithRateAndAccLimit(clock=t[0], freq_cutoff = 0.8, restraint_coefficient=3.0,
                                              freq_sampling = 100, max_rate = 1.0, max_neg_rate = 0.5, max_acc=1.0,
...
class GainSchedulerlinear():
    def __init__(self, p1: list, p2:list):
        """
        implement a simple linear scheduling  between 2 gains where:
        y = p1[1] if x<p1[0]
        y = p2[1] if x>p2[0]
        y = p1[1] + (p2[1]-p1[1])/(p2[0]-p1[0]) * (x-p1[1]) if p1[0] < x < p2[0]
        """
        self.p1 = p1
        self.p2 = p2
    ...
    def calc_gain(self, x):
        if x < self.p1[0]:
            ...
            return self.p1[1] + self.slope * (x - self.p1[0])
    def test(self):
        p1 = [15, 1]
        p2 = [25, 0.6]
        gain_scheduler_obj = GainSchedulerlinear(p1, p2)
        x = np.arange(0, 120, 1)
        y = []
        y2 = []
        for xi in x:
            y.append(gain_scheduler_obj.calc_gain(xi))
            if xi <= 15:
                y2.append(1.0)
            else:
                y2.append(15/(xi))
        ...
        plt.plot(x, y)
        plt.plot(x, y2)
        plt.legend(['implemented gain scheduling','1/speed'])
        plt.grid(True)
        plt.xlabel('KPH'),plt.ylabel('gain')
        plt.show()
class GainSchedulerInvSpeed():
    def __init__(self, speed_th):
        self.speed_th = speed_th
    def calc_gain(self, x):
        if x < self.speed_th:
            return 1.0
        else:
            return self.speed_th / x
    def test(self):
        speed_th = 15
        gain_scheduler_obj = GainSchedulerInvSpeed(speed_th)
        x = np.arange(0, 120, 1)
        y = []
        for xi in x:
            y.append(gain_scheduler_obj.calc_gain(xi))
...
if __name__ == '__main__':
    g_sch_obj = GainSchedulerInvSpeed(15)
    g_sch_obj = GainSchedulerInvSpeed(15)
    g_sch_obj.test()